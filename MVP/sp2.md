...
- 🔐 Expansão de Controle e Segurança<br>
  . Implementação do controle de acesso por níveis de usuário, garantindo permissões mais refinadas e segurança operacional;<br>
  . Estruturação de autenticação mais robusta para diferentes responsabilidades dentro do sistema.

- ⭐ Personalização e Flexibilidade<br>
  . Implementação da funcionalidade de favoritar normas cadastradas para melhorar organização individual e acesso rápido;<br>
  . Disponibilização de modificações estruturais via código-fonte, permitindo adaptações técnicas conforme necessidades específicas da empresa;

- 📜 Rastreabilidade e Auditoria<br>
 . Criação do histórico de alterações de normas, registrando modificações, responsáveis e datas;<br>
 . Desenvolvimento da visualização do histórico de alterações para maior transparência e controle administrativo;
